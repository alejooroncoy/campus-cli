...
function decodeCursor(cursor:string, pathname:string, groupId?:string, limit?:number):string {
  let url:URL;
  ...
  if(url.origin!==origin || url.pathname!==pathname || (groupId === undefined ? url.searchParams.has('group_id') : url.searchParams.get('group_id')!==groupId) || (limit !== undefined && url.searchParams.get('limit') !== String(limit))) throw new Error('Cursor Mendeley no permitido.');
  return url.toString();
}
export class MendeleyService {
  ...
  async list(limit=20,cursor?:string) {z.number().int().min(1).max(100).parse(limit);const r=await this.api(cursor?decodeCursor(cursor,'/documents',undefined,limit):'/documents?limit='+limit);return {documents:z.array(documentSchema).parse(r.data),hasMore:!!r.next,nextCursor:r.next?encodeCursor(r.next):null};}
  async listGroups(limit=20,cursor?:string) {z.number().int().min(1).max(100).parse(limit);const r=await this.api(cursor?decodeCursor(cursor,'/groups',undefined,limit):'/groups?limit='+limit);return {groups:z.array(groupSchema).parse(r.data),hasMore:!!r.next,nextCursor:r.next?encodeCursor(r.next):null};}
  async listGroupDocuments(groupId:string,limit=20,cursor?:string) {
    ...
    const r=await this.api(cursor?decodeCursor(cursor,'/documents',id,limit):'/documents?'+new URLSearchParams({group_id:id,limit:String(limit)}));
