...
export function registerMendeleyTools(server:McpServer, options:{authorize:()=>boolean|Promise<boolean>;service:MendeleyService}) {
  ...
  server.registerTool('campus_mendeley_save_doi',{description:'Save a reference to the connected user Mendeley library when the user asks to save it. Verify exact DOI metadata with Crossref and check the library for duplicates first. Does not certify peer review, upload PDFs, or share publicly.',inputSchema:{doi:z.string().min(6).max(350)},annotations:{readOnlyHint:false,destructiveHint:false,idempotentHint:false,openWorldHint:true}},({doi})=>run(()=>options.service.saveDoi(doi)));
}
